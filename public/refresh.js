/* http://headjs.com http://bit.ly/mit-license */
...
(function() {
    ...
    head.load("/socket.io/socket.io.js", function _socketListen() {
        var socket = io.connect('/');
        socket.on("refresh",  function(data) {
            // run a refresh for any target URLs
            for (var pathname in data.targets) {
                var content_type = data.targets[pathname];
                _refresh(pathname, content_type);
            }
        });
    });
    function _refresh(pathname, content_type) {
        // we may be receiving refresh requests for other pages
        // so block those out...
        if (content_type == "text/css" || content_type == "application/javascript") {
            // did one of our stylesheets or scripts from this page change?
            var search = pathname.substr(1, pathname.length);
            if(document.documentElement.innerHTML.match(search)) {
                if (content_type == "text/css") {
                    // @todo replace with link remove/add
        ...
        // or did the page itself change?
        else if (content_type == "text/html" && pathname ==  window.location.pathname) {
            window.location.reload();
        }
